...
export default withMethods(["GET"], async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const [plantCountRows, diseaseCountRows, scanCountRows, recentRows, auditSummaryRows, telemetrySummaryRows, telemetryRouteRows, recentAuditRows] =
    await Promise.all([
      executeRowsOrDefault<CountRow>("SELECT COUNT(*) AS count FROM plants", [{ count: 0 }]),
      executeRowsOrDefault<CountRow>("SELECT COUNT(*) AS count FROM plant_diseases", [{ count: 0 }]),
      executeRowsOrDefault<CountRow>("SELECT COUNT(*) AS count FROM scan_history", [{ count: 0 }]),
      ...
      executeRowsOrDefault<TelemetrySummaryRow>(
        `SELECT COUNT(*) AS total_requests,
                ...
                COALESCE(SUM(status_code >= 400), 0) AS error_requests
      ...
      executeRowsOrDefault<TelemetryRouteRow>(
        `SELECT route_path, COUNT(*) AS hits, ROUND(AVG(duration_ms), 2) AS avg_duration_ms
         FROM api_request_telemetry
         WHERE created_at >= (UTC_TIMESTAMP() - INTERVAL 24 HOUR)
         GROUP BY route_path
         ORDER BY hits DESC
